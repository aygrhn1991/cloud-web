...
import { G6DocResult, Result } from 'src/app/models/result.model';
...
export class VehModeComponent implements OnInit {
  ...
  getData(): void {
    this.loading = true;
    this.http.getG6DocVehMode().subscribe((data: Result) => {
      this.loading = false;
  ...
  handleOkAdd(): void {
    ...
    this.http.addG6DocVehMode(this.addModel).subscribe((data: Result) => {
      if (data.successed) {
