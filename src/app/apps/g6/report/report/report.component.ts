...
export class ReportComponent implements OnInit {
  ...
  loadDatas(yyyy: number = this.yyyys[0]): void {
    ...
    this.http.queryNowDocVeh(this.util.parameterTransfer(this.searchModel.vid, -1)).subscribe((data: Result) => {
      this.loading = false;
      this.docveh = data.data;
      console.log("===this.docveh==="+this.docveh)
    })

    this.http.queryVehException(this.util.parameterTransfer(this.searchModel.vid, -1), yyyy).subscribe((data: Result) => {
      this.loading = false;
