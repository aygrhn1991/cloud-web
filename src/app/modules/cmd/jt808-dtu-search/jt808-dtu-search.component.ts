...
@Component({
  selector: 'app-jt808-dtu-search',
  templateUrl: './jt808-dtu-search.component.html',
  styleUrls: ['./jt808-dtu-search.component.css']
})
export class Jt808DtuSearchComponent implements OnInit {
  @Input()
  vids = [];
  config = [
    {
  ...
  sendNormalCmd(i: number) {
    if (!this.vids || this.vids.length == 0) {
      this.cmdSvc.showMessage("请至少选择一台待发送指令的车辆");
      return;
    }
    const keys = [0x8104, 0x8107];
    this.cmdSvc.sendjt808NormalCmd({
      key: keys[i],
      vids: this.vids
    }, (data) => {
      console.log(data);
  ...
  sendSchCmd() {
    ...
    this.cmdSvc.sendjt808SchDtuConfig({
      items: cmds,
      vids: this.vids
    }, (data) => {
      console.log(data);
