...
export class VehDocComponent implements OnInit {
  ...
  getData(): void {
    this.loading = true;
    this.http.getG6DocVeh().subscribe((data: Result1) => {
      this.loading = false;
  ...
  handleOkAdd(): void {
    ...
    this.http.addG6DocVeh(this.tempId, this.addModel).subscribe((data: Result1) => {
      if (data.success) {
  ...
  handleOkEdit(): void {
    ...
    this.http.editG6DocVeh(this.editModel).subscribe((data: Result1) => {
      if (data.success) {
