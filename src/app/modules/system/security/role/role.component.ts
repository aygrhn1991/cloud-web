...
export class RoleComponent implements OnInit {
  ...
  loadDatas(reset: boolean = false): void {
    ...
    this.http.loadRoles().subscribe((data: Result2) => {
      this.loading = false;
  ...
  handleOk_add(): void {
    ...
    this.http.saveRole({ role: this.dataModel, ids: null }).subscribe((data: Result2) => {
      if (data.successed) {
  ...
  handleOk_edit(): void {
    ...
    this.http.saveRole({ role: this.dataModel, ids: ids }).subscribe((data: Result2) => {
      if (data.successed) {
        this.notification.success('保存成功', null);
        this.loadDatas();
        this.isVisible_edit = false;
  ...
  delete(): void {
    if (this.tempData == null) {
      ...
      this.modalService.confirm({
        ...
        nzOnOk: () => {
          this.http.delRole(this.tempData.id).subscribe ((data: Result2) => {
            if (data.successed) {
