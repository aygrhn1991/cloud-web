...
export class UserComponent implements OnInit {
  ...
  loadDatas(reset: boolean = false): void {
    ...
    this.http.queryUsers(this.util.parameterTransfer(this.form_name, '-1')).subscribe((data: Result2) => {
      this.loading = false;
  ...
  showModal_add(): void {
    ...
    this.http.loadAllRoles().subscribe((data: Result2) => {
  ...
  handleOk_add(): void {
    ...
    this.http.addUser({ user: this.dataModel, rids: ids }).subscribe((data: Result2) => {
      if (data.successed) {
        switch (data.data) {
          case 0:
  ...
  showModal_edit(): void {
    ...
    this.http.findUserRoles(this.dataModel.id).subscribe((data: Result2) => {
  ...
  handleOk_edit(): void {
    ...
    this.http.updateUser({ user: this.dataModel, rids: ids }).subscribe((data: Result2) => {
      if (data.successed) {
        switch (data.data) {
          case 0:
  ...
  //更改用户状态
  setState(e): void {
    this.http.updateUserStatus(e.id, ((e.status == 0 || e.status == 4) ? 1 : 0)).subscribe((data: Result2) => {
      if (data.successed) {
        switch (data.data) {
          case 0:
            this.notification.error('保存失败', null);
